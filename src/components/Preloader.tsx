...
export function Preloader({ onFinish }: PreloaderProps) {
  ...
  const timeoutEsperaRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const timeoutFadeRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const ultimoProgressoRef = useRef(-1);
  ...
  useEffect(() => {
    ...
    if (reduzMovimento) {
      ...
      if (aguaRef.current) {
        aguaRef.current.style.transform = "translate3d(0, 18vh, 0)";
      }
    ...
    function passo(agora: number) {
      ...
      const topoAgua = 100 - suavizado * 82;

      if (aguaRef.current) {
        aguaRef.current.style.transform = `translate3d(0, ${topoAgua}vh, 0)`;
      }

      const percentual = Math.round(suavizado * 100);

      if (percentual !== ultimoProgressoRef.current) {
        ultimoProgressoRef.current = percentual;
      ...
      if (t < 1) {
        ...
        if (aguaRef.current) {
          aguaRef.current.style.transform =
            "translate3d(0, 18vh, 0)";
        }

        setProgresso(100);
        ultimoProgressoRef.current = 100;
